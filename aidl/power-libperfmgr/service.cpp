...
int main() {
    ...
    if (android::base::GetBoolProperty(kConfigDebugPathProperty.data(), false)) {
        ...
        LOG(WARNING) << "Moto Power HAL AIDL Service is using debug config from: " << config_path;
    }
    config_path.append(
            android::base::GetProperty(kConfigProperty.data(), kConfigDefaultFileName.data()));

    LOG(INFO) << "Moto Power HAL AIDL Service with Extension is starting with config: "
              << config_path;
    ...
    LOG(INFO) << "Moto Power HAL AIDL Service with Extension is started.";
    ...
    // should not reach
    LOG(ERROR) << "Moto Power HAL AIDL Service with Extension just died.";
    return EXIT_FAILURE;
}
